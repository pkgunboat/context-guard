...
├── summarizing                    # 总结模式状态文件（内容是本次 prompt 文件路径）
...
1. **PreToolUse hook / Stop hook** → 检查上下文是否超过阈值
2. **Hook 写入确定的目标文件名** → `~/.claude/context-guard/summarizing`
3. **Claude 保存 prompt** → `~/.claude/context-guard/continuation_<session_id>.md`
4. **Stop hook 写标记** → `echo "路径" > ~/.claude/context-guard/next_prompt`
...
或者按自己的安全偏好使用 Claude Code 的权限参数，例如预授权写文件相关工具。不要在不信任的仓库或命令环境中使用 `--dangerously-skip-permissions`。
